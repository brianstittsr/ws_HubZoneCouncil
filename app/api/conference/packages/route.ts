...
export async function GET(req: NextRequest) {
  try {
    ...
    const col = collection(db, COLLECTIONS.CONFERENCE_SPONSORSHIP_PACKAGES);
    ...
    console.error("GET /api/conference/packages:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const docRef = await addDoc(collection(db, COLLECTIONS.CONFERENCE_SPONSORSHIP_PACKAGES), {
      ...body,
      currentSponsors: body.currentSponsors ?? 0,
    ...
    console.error("POST /api/conference/packages:", error);
